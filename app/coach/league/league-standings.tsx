...
import type { LeagueStanding } from "@/lib/mockData"
...
export function LeagueStandings({ standings }: LeagueStandingsProps) {
  ...
  const coachTeamPosition = standings.find(team => team.team === coachTeam)?.position || 0
    ...
    <Card>
      <CardHeader>
        ...
        <div className="flex items-center gap-2">
          {coachTeam && (
            <Badge variant={coachTeamPosition <= 3 ? "default" : "secondary"}>
              {coachTeam} - Position {coachTeamPosition}
            </Badge>
          )}
        </div>
      </CardHeader>
              ...
              {standings.map(team => (
                <tr
                  ...
                  className={`border-b hover:bg-muted/50 ${
                    team.team === coachTeam ? "bg-primary/10 font-medium" : ""
                  }`}
                >
