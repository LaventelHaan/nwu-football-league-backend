...
DROP DATABASE IF EXISTS NWU_Sports_League;
CREATE DATABASE IF NOT EXISTS NWU_Sports_League
...
CREATE TABLE IF NOT EXISTS roles (
  ...
  role_key      VARCHAR(32) NOT NULL UNIQUE,             -- 'ADMIN','COACH','SCOUT','PLAYER'
...
-- Equipment stock: prevent negative & auto-apply delta
DROP TRIGGER IF EXISTS trg_equipment_movements_guard;
DELIMITER $$
CREATE TRIGGER trg_equipment_movements_guard
BEFORE INSERT ON equipment_movements FOR EACH ROW
BEGIN
  DECLARE cur_qty INT;
  SELECT quantity INTO cur_qty FROM team_equipment_stock WHERE stock_id = NEW.stock_id FOR UPDATE;
  IF cur_qty IS NULL THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Invalid stock_id for equipment movement.';
  END IF;
  IF cur_qty + NEW.delta_qty < 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Insufficient stock for this movement.';
  END IF;
END$$
DELIMITER ;

DROP TRIGGER IF EXISTS trg_equipment_movements_apply;
DELIMITER $$
CREATE TRIGGER trg_equipment_movements_apply
AFTER INSERT ON equipment_movements FOR EACH ROW
BEGIN
  UPDATE team_equipment_stock SET quantity = quantity + NEW.delta_qty WHERE stock_id = NEW.stock_id;
END$$
DELIMITER ;
...
INSERT IGNORE INTO roles (role_id, role_key, role_name) VALUES
  (1,'ADMIN','Administrator'),(2,'COACH','Coach'),(3,'SCOUT','Scout'),(4,'PLAYER','Player');

INSERT IGNORE INTO approval_status (status_key, description) VALUES
  ('PENDING','Awaiting review'),('APPROVED','Approved by admin'),('REJECTED','Rejected by admin');

INSERT IGNORE INTO invite_status (status_key, description) VALUES
  ('SENT','Invitation sent'),('ACCEPTED','Accepted'),('DECLINED','Declined'),('EXPIRED','Expired'),('CANCELLED','Cancelled');

INSERT IGNORE INTO match_status (status_key, description) VALUES
  ('SCHEDULED','Not started'),('IN_PROGRESS','Live'),('FINAL','Completed'),('POSTPONED','Postponed'),('CANCELLED','Cancelled');

INSERT IGNORE INTO booking_type (type_key, description) VALUES
  ('TRAINING','Team training'),('TRYOUT','Player tryout'),('MATCH','Fixture/match');

INSERT IGNORE INTO booking_status (status_key, description) VALUES
  ('REQUESTED','Awaiting approval'),('APPROVED','Approved'),('REJECTED','Rejected'),('CANCELLED','Cancelled'),('COMPLETED','Completed');

INSERT IGNORE INTO player_position (position_key, description) VALUES
...
INSERT IGNORE INTO event_type (event_key, description) VALUES
  ...
  ('OWN_GOAL','Own goal'),('SUB_ON','Substitution on'),('SUB_OFF','Substitution off'),
  ('PENALTY_GOAL','Penalty scored'),('PENALTY_MISS','Penalty missed'),
  ('SAVE','Goalkeeper save'),('CLEAN_SHEET','Clean sheet');

INSERT IGNORE INTO notification_type (type_key, description) VALUES
  ('TRIAL_INVITE','Trial invitation'),('MATCH_UPDATE','Fixture/match update'),
  ('ANNOUNCEMENT','Announcement'),('BOOKING_UPDATE','Field booking update');
